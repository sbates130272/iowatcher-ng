use std::{error::Error, fs::File, io::BufReader, net::SocketAddr, path::PathBuf};
...
pub async fn serve(cert: &PathBuf, key: &PathBuf, port: &u16) -> Result<(), Box<dyn Error>> {
    PrometheusBuilder::new()
        .with_http_listener("[::]:9975".parse::<SocketAddr>()?)
        .install()?;

    let (certs, privkey) = read_certs_from_file(cert, key).expect("valid cert and key paths");
